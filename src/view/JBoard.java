		...
		if (p != -1) {
			...
			default:
			 ...
			 try {
	                ...
	                String imagePath = getImage(row, col);

	                if (imagePath != null) {

	                    URL imageURL = Utils.class.getClassLoader().getResource(imagePath);

	                    if (imageURL != null) {

	                        BufferedImage image = ImageIO.read(imageURL);

	                        g.drawImage(image, x + _SEPARATOR * 2, y + _SEPARATOR * 2, (_CELL_WIDTH - 2 * _SEPARATOR - 4), 
	                                             (_CELL_HEIGHT - 2 * _SEPARATOR - 4), null);
